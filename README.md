...
A relational database is a database that stores information in tables, SQL is the software or language which we use to access that information from the database.

- [ ] Why do tables need a `primary key`?
In order to indentify each entry into the database uniquely

- [ ] What is the name given to a table column that references the primary key on another table.
A foreign key

- [ ] What do we need in order to have a _many to many_ relationship between two tables.
An intermediary table with foreign keys for both tables
